...
const string Stack::StackError = "stackError";
...
void Stack::StackImpl::swapTop() {
    if (stack_.size() < 2) {
        parent_.raise(Stack::StackError, std::make_shared<StackEventData>(StackEventData::ErrorConditions::TooFewArguments))
...
void Stack::StackImpl::getElements(size_t n, vector<double>& v) const {
    // if n is > stack's size, return size of stack
    if (n > stack_.size()) {
        n = stack_.size();
    }
    // using .insert method of std::vector 
    // Inserts elements from range [first, last) before pos. 
    // insert(pos, first, last)
    // v.end(): past the end iterator
...
Stack& Stack::Instance() {
    static Stack instance;    // recall there is only one instance of stack
    return instance;
}

void Stack::push(double d, bool suppressChangeEvent) {
    return pimpl_->push(d, suppressChangeEvent);
}

void Stack::pop(bool suppressChangeEvent) {
...
vector<double> Stack::getElements(size_t n, vector<double>& v) const {
